...
bool qbuDatabase::verifyCoverage( qbuStringList & lstSucceeded, qbuStringList & lstFailed )
{
	...
	if (retVal) {
		...
		if (!retVal) {
			...
			QLOG_WARN_ASSERT() << QBULOG_DATABASE_TYPE << strMsg;
		}
	}
	return retVal;
}
...
bool qbuDatabase::forceUpdateViews()
{
	return true;
}
...
QString qbuDatabase::getDefaultAttachName()
{
	QString retVal;
...
std::shared_ptr<qbuDBSettingsTableBase> qbuDatabase::getSettingsTable()
{
	std::shared_ptr<qbuDBSettingsTableBase> retVal;
