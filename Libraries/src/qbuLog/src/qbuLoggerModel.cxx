...
public:
	...
	bool    calculateRowsToRemove(LogQueue::const_iterator& start, LogQueue::const_iterator& finish, quint32& nRemove)
	    const;
...
qbuLoggerModel::qbuPrivate::qbuPrivate()
    : m_pEngine{new qbuLogModelEngine}, m_nRecordLimit{100 * 1024}, m_nUpdateDelay{5} {
...
bool qbuLoggerModel::qbuPrivate::calculateRowsToRemove(
    LogQueue::const_iterator& start, LogQueue::const_iterator& finish, quint32& nRemove
) const {
...
quint32 qbuLoggerModel::qbuPrivate::resizeQueue(qbuLoggerModel* pPublic) {
	...
	if (calculateRowsToRemove(start, finish, retVal)) {
		...
		// The surviving rows have shifted down by retVal positions.
		// Their cached heights are still valid for the content, but the
		// view's section indices no longer match — invalidate so
		// updateRowHeights() re-registers them with the view on next access.
		for (auto& entry : m_queue) {
			entry.m_cachedHeight = -1;
		}
	}
...
qbuLoggerModel::qbuLoggerModel(QObject* pParent) : m_pPrivate(std::make_unique<qbuPrivate>()) {
	connect(
	    m_pPrivate->m_pEngine, SIGNAL(logMessage(QDateTime, quint32, QString, quint32, const QStringList&)), this,
	    SLOT(logMessage(QDateTime, quint32, QString, quint32, const QStringList&))
	);
...
void qbuLoggerModel::logMessage(
    QDateTime dt, quint32 level, QString strFileName, quint32 nLine, const QStringList& messages
) {
...
void qbuLoggerModel::synchronize() {
	...
	if (nQueueSize > 0) {
		...
		std::move(
		    begin(m_pPrivate->m_buffer.m_holdQueue), end(m_pPrivate->m_buffer.m_holdQueue),
...
QVariant qbuLoggerModel::data(const QModelIndex& index, int role /*= Qt::DisplayRole*/) const {
	...
	if (role == Qt::UserRole) {
		if (index.column() == CT_LEVEL) {
			if ((index.row() < (int)m_pPrivate->m_queue.size()) && (index.column() < GRID_COLS)) {

				const logData& item = m_pPrivate->m_queue[index.row()];
		...
		if (index.column() == CT_MESSAGE && index.row() < (int)m_pPrivate->m_queue.size()) {
			int h = m_pPrivate->m_queue[index.row()].m_cachedHeight;
			if (h > 0)
				retVal = QSize(0, h);
			// Return invalid QVariant when unmeasured — the view will call
			// sizeHintForIndex(), and updateRowHeights() writes the result
			// back via setRowHeight().
		}
		...
		if ((index.row() < (int)m_pPrivate->m_queue.size()) && (index.column() < GRID_COLS)) {
			...
			// 			case CT_FILENAME:
			// 				retVal =
			// QString("%1:%2").arg(m_pPrivate->lookupFileNameFromIndex(item.m_nFileIndex)).arg(item.m_nFileLineNumber);
			// 				break;
			case CT_MESSAGE:
		...
		case CT_FILENAME:
			if (role == Qt::ToolTipRole) {
				...
				// EditRole: plain "filename:line" — this is what gets copied to the clipboard.
				retVal = QString("%1:%2")
				             .arg(m_pPrivate->lookupFileNameFromIndex(item.m_nFileIndex))
				             .arg(item.m_nFileLineNumber);
			}
...
int qbuLoggerModel::rowHeight(int row) const {
	if (row < 0 || row >= (int)m_pPrivate->m_queue.size())
		return -1;
	return m_pPrivate->m_queue[row].m_cachedHeight;
}
...
void qbuLoggerModel::setRowHeight(int row, int height) {
	if (row < 0 || row >= (int)m_pPrivate->m_queue.size())
		return;

	// Allow explicit invalidation via -1 even if the value appears unchanged.
	if (height != -1 && m_pPrivate->m_queue[row].m_cachedHeight == height)
		return;

	m_pPrivate->m_queue[row].m_cachedHeight = height;

	if (height > 0) {
		QModelIndex idx = index(row, CT_MESSAGE);
		emit        dataChanged(idx, idx, {Qt::SizeHintRole});
	}
}
